            ...
            paymentToken:(NSString *_Nullable)token
{
    ...
    if (payment.token.paymentMethod.type == PKPaymentMethodTypeUnknown) {
        paymentMethodType = @"unknown";
    } else if (payment.token.paymentMethod.type == PKPaymentMethodTypeDebit) {
        paymentMethodType = @"debit";
    } else if (payment.token.paymentMethod.type == PKPaymentMethodTypeCredit) {
        paymentMethodType = @"credit";
    } else if (payment.token.paymentMethod.type == PKPaymentMethodTypePrepaid) {
        paymentMethodType = @"prepaid";
    } else if (payment.token.paymentMethod.type == PKPaymentMethodTypeStore) {
        paymentMethodType = @"store";
    }

    NSString *paymentMethodDisplayName = payment.token.paymentMethod.displayName;
    NSString *paymentMethodNetwork = payment.token.paymentMethod.network;
    ...
    NSError *error;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:paymentMethodDict options:0 error:&error];
    NSString *paymentMethod;
    if (! jsonData) {
        paymentMethod = @"";
    } else {
        paymentMethod = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
    }
